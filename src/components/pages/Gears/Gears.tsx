...
export default function Gears() {
  ...
  const { data, isLoading } = useQuery<CraftsQuery>(
    [
      "gears",
      {
        ...filters,
      ...
      <StyledGears>
        <Header>
          ...
          <ColumnHeader onClick={() => toggleFilter("craft")}>
            Craft
            {frontFilters.craft === "asc"
              ? " ^"
              : frontFilters.craft === "desc"
              ? " v"
              : ""}
          </ColumnHeader>
          <span>Ratio</span>
        ...
        {isLoading ? (
          <p>ça charge les cocos</p>
        ) : (
          sortedGears(filteredGears).map((gear) => (
            <Gear
              key={Object.values(gear).join("")}
              data={gear}
              afterUpdate={refetch}
            />
          ))
  ...
  function setAllSold() {
    sellMany(gears.map(({ _id }) => _id)).then(refetch);
  }
  ...
  function newFilterValue(
    filter: keyof typeof frontFilters,
    value: "off" | "asc" | "desc"
  ) {
    switch (value) {
      case "off":
  ...
  function onBulkUpdateChange(event: ChangeEvent<HTMLInputElement>) {
    if (getBulkUpdateParametersPropertyNames().includes(event.target.name)) {
      const parameters: BulkUpdateParameters = {

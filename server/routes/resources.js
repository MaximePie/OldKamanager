...
import {get, fill, update, getPricesHistory} from '../controllers/resources.js';
...
router.get('/required/fill/', async (request, response) => {
    ...
    resources.map(async (resource) => {
      ...
      const totalQuantity = gearsWhereTheResourceIsNeeded.reduce(
        (accumulator, currentValue) =>
          accumulator + currentValue.recipe
            .find(({name: recipeResourceName}) => recipeResourceName === name)
            ?.quantity
        , 0)
  ...
  response.json({
    updatedResources,
    quantities: updatedResources?.map(({name, timesRequiredInRecipes}) => ({name, timesRequiredInRecipes})),

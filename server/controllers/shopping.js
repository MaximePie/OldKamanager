...
const calculateTotalSlots = (list) => {
  const slots = list.reduce((acc, { toBeCrafted }) => {
    return acc + toBeCrafted;
  }, 0);
...
export async function get(request, response) {
  const craftTargetQuery = Gear.find({
    toBeCrafted: {
      $gt: 0,
    },
  }).sort({
  ...
  response.json({
    ...
    estimatedIncome,
    slots: calculateTotalSlots(craftTarget),
  });
}
